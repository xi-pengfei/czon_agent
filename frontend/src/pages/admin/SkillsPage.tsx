...
export function SkillsPage() {
  ...
  async function upload(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const result = await api<{ name: string }>("/api/admin/skills/upload", { method: "POST", body });
      setNotice(`${result.name} 已上传`); await load();
    } catch (error) { setNotice((error as Error).message); }
    finally { setBusy(""); }
  }
  ...
  return <main className="admin-content">
    <PageHeader title="Skills 管理" description="管理服务器上的企业专属 Skills" action={<div className="page-actions">
    ...
    <div className="table-toolbar"><span>共 {skills.length} 个 Skill</span><span>删除前必须先停用</span></div>
    <div className="data-table"><table><thead><tr><th>Skill</th><th>目录</th><th>大小</th><th>更新时间</th><th>状态</th><th>操作</th></tr></thead><tbody>{skills.map((skill) => <tr key={skill.name}>
      ...
      <td><button className="text-button danger" disabled={skill.enabled || busy === skill.name} title={skill.enabled ? "请先停用 Skill" : "删除 Skill"} onClick={() => void remove(skill)}><Trash2 size={14} />删除</button></td>
