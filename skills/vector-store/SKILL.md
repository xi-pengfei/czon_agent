...
首次部署运行 `bash scripts/install_qdrant.sh`，把 Qdrant 二进制下载到项目内的 `.runtime/qdrant/bin/qdrant`。
之后启动 Agent（WebUI、交互 CLI、单次 CLI）时会自动确保 Qdrant 在线，数据存于项目内的 `data/qdrant/`。
